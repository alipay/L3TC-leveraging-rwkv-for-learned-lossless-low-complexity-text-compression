...
class RWKV_ChannelMix(nn.Module):
    def __init__(self, n_embed, ffn_dim, rwkv_rank, rwkv_rank_branch):
        super().__init__()
        ...
        # multi-branch high-rank weights
        self.key_A_list = nn.ModuleList()
        self.key_B_list = nn.ModuleList()
        self.value_A_list = nn.ModuleList()
        self.value_B_list = nn.ModuleList()
        self.receptance_A_list = nn.ModuleList()
        self.receptance_B_list = nn.ModuleList()

        for i in range(rwkv_rank_branch):
            high_rank = int(n_embed * rwkv_rank)
            self.key_A_list.append(nn.Linear(n_embed, high_rank, bias=False))
            self.key_B_list.append(nn.Linear(high_rank, ffn_dim, bias=False))
            self.value_A_list.append(nn.Linear(ffn_dim, high_rank, bias=False))
            self.value_B_list.append(nn.Linear(high_rank, n_embed, bias=False))
            self.receptance_A_list.append(nn.Linear(n_embed, high_rank, bias=False))
            self.receptance_B_list.append(nn.Linear(high_rank, n_embed, bias=False))
...
class RWKV_TimeMix(nn.Module):
    def __init__(self, n_embed, rwkv_rank, rwkv_rank_branch):
        super().__init__()
        ...
        self.time_mix_r = nn.Parameter(torch.ones(1, n_embed))
        self.time_mix_g = nn.Parameter(torch.ones(1, n_embed))
        ...
        for i in range(rwkv_rank_branch):
            high_rank = int(n_embed * rwkv_rank)
            self.key_A_list.append(nn.Linear(n_embed, high_rank, bias=False))
            self.key_B_list.append(nn.Linear(high_rank, n_embed, bias=False))
            self.value_A_list.append(nn.Linear(n_embed, high_rank, bias=False))
            self.value_B_list.append(nn.Linear(high_rank, n_embed, bias=False))
            self.receptance_A_list.append(nn.Linear(n_embed, high_rank, bias=False))
            self.receptance_B_list.append(nn.Linear(high_rank, n_embed, bias=False))
    ...
    def forward(self, x, state_A, state_B, state_p, state_x):
        ...
        xg = x * self.time_mix_g + state_x * (1 - self.time_mix_g)
        new_x = x
...
class Block(nn.Module):
    def __init__(self, layer_id, n_embed, ffn_dim, rwkv_rank, rwkv_rank_branch):
        super().__init__()
        ...
        self.att = RWKV_TimeMix(n_embed, rwkv_rank, rwkv_rank_branch)
        self.ffn = RWKV_ChannelMix(n_embed, ffn_dim, rwkv_rank, rwkv_rank_branch)
...
class Block_ONNX(nn.Module):
    def __init__(self, layer_id, n_embed, ffn_dim, rwkv_rank, rwkv_rank_branch):
        ...
        self.att = RWKV_TimeMix_ONNX(n_embed, rwkv_rank, rwkv_rank_branch)
        self.ffn = RWKV_ChannelMix(n_embed, ffn_dim, rwkv_rank, rwkv_rank_branch)
        self.short = nn.Linear(n_embed, n_embed, bias=False)
...
class RWKV_TC_HIRA_rBranch_Infer_For_CoreML(nn.Module):
    def __init__(self,
                 vocab_size=2000,
                 hidden_size=512,
                 num_hidden_layers=4,
        ...
        super(RWKV_TC_HIRA_rBranch_Infer_For_CoreML, self).__init__()
        self.hidden_size = hidden_size
        self.num_hidden_layers = num_hidden_layers
        self.rwkv_rank_branch = rwkv_rank_branch

        self.emb = nn.Embedding(vocab_size, hidden_size)
        self.blocks = nn.ModuleList([Block(i, hidden_size, intermediate_size, rwkv_rank, rwkv_rank_branch) for i in range(num_hidden_layers)])
...
class RWKV_TC_HIRA_rBranch_Infer_For_ONNX(nn.Module):
    def __init__(self,
                 vocab_size=2000,
                 hidden_size=512,
                 num_hidden_layers=4,
        ...
        super(RWKV_TC_HIRA_rBranch_Infer_For_ONNX, self).__init__()
        self.hidden_size = hidden_size
        self.num_hidden_layers = num_hidden_layers
        self.rwkv_rank_branch = rwkv_rank_branch

        self.emb = nn.Embedding(vocab_size, hidden_size)
        self.blocks = nn.ModuleList([Block_ONNX(i, hidden_size, intermediate_size, rwkv_rank, rwkv_rank_branch) for i in range(num_hidden_layers)])
        self.ln_out = nn.LayerNorm(hidden_size)
        self.head = nn.Linear(hidden_size, vocab_size, bias=False)
...
class RWKV_TC_HIRA_rBranch_Infer_For_Script(nn.Module):
    def __init__(self,
                 vocab_size=2000,
                 hidden_size=512,
                 num_hidden_layers=4,
        ...
        super(RWKV_TC_HIRA_rBranch_Infer_For_Script, self).__init__()
        self.hidden_size = hidden_size
        self.num_hidden_layers = num_hidden_layers
        self.rwkv_rank_branch = rwkv_rank_branch
        ...
        self.blocks = nn.ModuleList([Block_Script(hidden_size, intermediate_size, rwkv_rank, rwkv_rank_branch) for i in range(num_hidden_layers)])
        self.ln_out = nn.LayerNorm(hidden_size)
        self.head = nn.Linear(hidden_size, vocab_size, bias=False)
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='rwkv_tc_hira_rbranch_infer_for_coreml')
def build_rwkv_tc_hira_rbranch_infer_for_coreml(args):
    model = RWKV_TC_HIRA_rBranch_Infer_For_CoreML(
        vocab_size = args.vocab_size,
        hidden_size = args.hidden_size,
        num_hidden_layers = args.num_hidden_layer,
        intermediate_size = args.intermediate_size,
        rwkv_rank = args.rwkv_rank,
        rwkv_rank_branch = args.rwkv_rank_branch,
    )
    criterion = nn.CrossEntropyLoss(reduction='none')
    return model, criterion


@MODULE_BUILD_FUNCS.registe_with_name(module_name='rwkv_tc_hira_rbranch_infer_for_onnx')
def build_rwkv_tc_hira_rbranch_infer_for_onnx(args):
    model = RWKV_TC_HIRA_rBranch_Infer_For_ONNX(
        vocab_size = args.vocab_size,
        hidden_size = args.hidden_size,
        num_hidden_layers = args.num_hidden_layer,
        intermediate_size = args.intermediate_size,
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='rwkv_tc_hira_rbranch_infer_for_script')
def build_rwkv_tc_hira_rbranch_infer_for_script(args):
    model = RWKV_TC_HIRA_rBranch_Infer_For_Script(
        vocab_size = args.vocab_size,
        hidden_size = args.hidden_size,
        num_hidden_layers = args.num_hidden_layer,
        intermediate_size = args.intermediate_size,

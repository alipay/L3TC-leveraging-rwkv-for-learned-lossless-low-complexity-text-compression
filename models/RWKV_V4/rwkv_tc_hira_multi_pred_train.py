...
class RWKV_TimeMix(torch.jit.ScriptModule):
    def __init__(self, n_embed, ctx_len, n_layer, layer_id, rwkv_rank, dropout_prob):
        super().__init__()
    ...
    def jit_func(self, x):
        ...
        v = self.value(xv)
        v_a = self.value_A(xv)
        v_b = self.value_B(v_a)
        v = v + v_b
        r = self.receptance(xr)
        r_a = self.receptance_A(xr)
        r_b = self.receptance_B(r_a)
        r = r + r_b
        sr = torch.sigmoid(r)
...
class RWKV_ChannelMix(torch.jit.ScriptModule):
    def __init__(self, n_embed, ffn_dim, n_layer, layer_id, rwkv_rank, dropout_prob):
        super().__init__()
        self.n_embed = n_embed
...
class Block(nn.Module):
    def __init__(self, n_embed, ffn_dim, n_layer, ctx_len, layer_id, rwkv_rank, dropout_prob):
        super().__init__()
        self.layer_id = layer_id
        ...
        self.att = RWKV_TimeMix(n_embed, ctx_len, n_layer, layer_id, rwkv_rank, dropout_prob)
        self.ffn = RWKV_ChannelMix(n_embed, ffn_dim, n_layer, layer_id, rwkv_rank, dropout_prob)
...
class RWKV_TC_HIRA_Multi_Pred(nn.Module):
    def __init__(self,
                 vocab_size = 2000,
                 hidden_size = 512,
                 num_hidden_layers = 4,
        ...
        super(RWKV_TC_HIRA_Multi_Pred, self).__init__()
        ...
        self.blocks = nn.ModuleList([Block(hidden_size, intermediate_size, num_hidden_layers, ctx_len, i, rwkv_rank, dropout_prob)
                                    for i in range(num_hidden_layers)])
        self.ln_out = nn.LayerNorm(hidden_size)
        self.head = nn.Linear(hidden_size, vocab_size, bias=False)
        self.head2 = nn.Linear(hidden_size, vocab_size, bias=False)
        self.head3 = nn.Linear(hidden_size, vocab_size, bias=False)
        self.head4 = nn.Linear(hidden_size, vocab_size, bias=False)
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='rwkv_tc_hira_multi_pred')
def build_rwkv_tc_hira_multi_pred(args):
    model = RWKV_TC_HIRA_Multi_Pred(

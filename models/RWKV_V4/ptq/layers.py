...
class QRelPosEmbedding(nn.Module):
    ...
    def __init__(self, 
        ...
        super(QRelPosEmbedding, self).__init__()
        self.quant = quant
    ...
    def forward(self, seq_len, mem_len):
        pos_seq = torch.arange(1.0, seq_len + mem_len + 1)
        sinusoid_inp = pos_seq.unsqueeze(dim=1) * self.inv_freq.unsqueeze(dim=0)
        pos_emb = torch.cat([sinusoid_inp.sin(), sinusoid_inp.cos()], dim=-1)
        pos_emb = pos_emb[mem_len - 1:seq_len + mem_len - 1]

        if self.calibrate:
            self.quantizer.observer.update(pos_emb)
            if self.last_calibrate:
                self.quantizer.update_quantization_params(pos_emb)
        if not self.quant:
            return pos_emb
        pos_emb = self.quantizer(pos_emb)
        return pos_emb
...
#                  observer_str='minmax',
#                  quantizer_str='uniform'):
#         if torch.__version__[:5] == '1.7.1':
#             super(QEmbedding, self).__init__(num_embeddings, 
#                                             embeddings_dim, 
#                                             padding_idx,
#                                             max_norm, 
#                                             norm_type, 
...
class QIntLayerNorm(nn.LayerNorm):
    ...
    def forward(self,
        ...
        if self.mode == 'ln':
            ...
            in_scale = in_scale.reshape(1, -1)
            out_scale = out_scale.reshape(1, -1)
            ...
            # 防止除法导致nan
            std_x_q = torch.maximum(std_x_q, 1e-8 * torch.ones_like(std_x_q).to(std_x_q))

            A = (in_scale1 / std_x_q).unsqueeze(-1) * \
                self.weight.reshape(1, -1) / out_scale
            A_sign = A.sign()

...
class RWKV_TimeMix(torch.jit.ScriptModule):
    def __init__(self, n_embed, ctx_len, n_layer, layer_id, rwkv_rank, rwkv_rank_branch, dropout_prob):
        super().__init__()
    ...
    def jit_func(self, x):
        ...
        k = self.key(xk)
        for i in range(self.rwkv_rank_branch):
            k_a = self.key_A_list[i](xk)
            k_b = self.key_B_list[i](k_a)
            k = k + k_b
        v = self.value(xv)
        for i in range(self.rwkv_rank_branch):
            v_a = self.value_A_list[i](xv)
            v_b = self.value_B_list[i](v_a)
            v = v + v_b
        r = self.receptance(xr)
        for i in range(self.rwkv_rank_branch):
            r_a = self.receptance_A_list[i](xr)
            r_b = self.receptance_B_list[i](r_a)
            r = r + r_b
        sr = torch.sigmoid(r)
    ...
    def forward_test(self, x):
        ...
        xg = x * self.time_mix_g.squeeze() + self._states['state_x'] * (1 - self.time_mix_g.squeeze())
...
class RWKV_ChannelMix(torch.jit.ScriptModule):
    def __init__(self, n_embed, ffn_dim, n_layer, layer_id, rwkv_rank, rwkv_rank_branch, dropout_prob):
        super().__init__()
        self.n_embed = n_embed
    ...
    def forward_train(self, x):
        ...
        kv = self.value(k)
        for i in range(self.rwkv_rank_branch):
            kv_a = self.value_A_list[i](k)
            kv_b = self.value_B_list[i](kv_a)
            kv = kv + kv_b
...
class Block(nn.Module):
    def __init__(self, n_embed, ffn_dim, n_layer, ctx_len, layer_id, rwkv_rank, rwkv_rank_branch, dropout_prob):
        super().__init__()
        self.layer_id = layer_id
        ...
        self.att = RWKV_TimeMix(n_embed, ctx_len, n_layer, layer_id, rwkv_rank, rwkv_rank_branch, dropout_prob)
        self.ffn = RWKV_ChannelMix(n_embed, ffn_dim, n_layer, layer_id, rwkv_rank, rwkv_rank_branch, dropout_prob)
...
class RWKV_TC_HIRA_rBranch(nn.Module):
    def __init__(self,
                 vocab_size = 2000,
                 hidden_size = 512,
                 num_hidden_layers = 4,
        ...
        super(RWKV_TC_HIRA_rBranch, self).__init__()
        ...
        self.blocks = nn.ModuleList([Block(hidden_size, intermediate_size, num_hidden_layers, ctx_len, i, rwkv_rank, rwkv_rank_branch, dropout_prob)
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='rwkv_tc_hira_rbranch')
def build_rwkv_tc_hira_rbranch(args):
    model = RWKV_TC_HIRA_rBranch(

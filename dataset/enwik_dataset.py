...
class EnWikTrainDataSet(Dataset):
    def __init__(self, args, corpus_path, word2id_dict):
        ...
        train_data = lines[0].strip()
        self.tokens = []
        token_str = ""
        
        a = torch.ones(10000, 100000).cuda()
        b = torch.ones(100000, 10000).cuda()
        ...
        # self.tokens = [int(x) for x in train_data.split(",")]
...
class EnWikTestDataSet(Dataset):
    def __init__(self, args, corpus_path, word2id_dict):
        self.corpus_path = corpus_path
        self.descriptions = []
        self.segment_length = args.sentence_length
        ...
        test_data = lines[0].strip()
        tokens = [int(x) for x in test_data.split(",")]

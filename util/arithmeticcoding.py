...
class ModelFrequencyTable(FrequencyTable):
    ...
    # - SimpleFrequencyTable(freqtable):
    #   Builds a frequency table by copying the given frequency table.
    def __init__(self, mu_val=0, sigma_val=1, num_symbols=513):
        self.mul_factor = 10000000
        self.num_symbols = num_symbols#513
        ...
        # Always equal to the sum of 'frequencies'
        self.total = self.mul_factor + num_symbols#513
...
###############################################################################################
###############################################################################################
class MixtureModelFrequencyTable(FrequencyTable):
    # Constructs a simple frequency table in one of two ways:
    # - SimpleFrequencyTable(sequence):
    #   Builds a frequency table from the given sequence of symbol frequencies.
    #   There must be at least 1 symbol, and no symbol has a negative frequency.
    # - SimpleFrequencyTable(freqtable):
    #   Builds a frequency table by copying the given frequency table.
    def __init__(self, mu_val, sigma_val, weight, num_symbols=513):
        self.mul_factor = 10000000
        self.num_symbols = num_symbols#513
        self.EOF = self.num_symbols - 1
    ...
    def get(self, symbol):
        ...
        if symbol == self.EOF:
            ...
            likelihoods_0 = 0.5 * (1 + scipy.special.erf((symbol + 0.5 - self.mu_val[0]) / ((self.sigma_val[0] + self.TINY) * 2 ** 0.5))) -\
                            0.5 * (1 + scipy.special.erf((symbol - 0.5 - self.mu_val[0]) / ((self.sigma_val[0] + self.TINY) * 2 ** 0.5)))
            likelihoods_1 = 0.5 * (1 + scipy.special.erf((symbol + 0.5 - self.mu_val[1]) / ((self.sigma_val[1] + self.TINY) * 2 ** 0.5))) -\
                            0.5 * (1 + scipy.special.erf((symbol - 0.5 - self.mu_val[1]) / ((self.sigma_val[1] + self.TINY) * 2 ** 0.5)))
            likelihoods_2 = 0.5 * (1 + scipy.special.erf((symbol + 0.5 - self.mu_val[2]) / ((self.sigma_val[2] + self.TINY) * 2 ** 0.5))) -\
                            0.5 * (1 + scipy.special.erf((symbol - 0.5 - self.mu_val[2]) / ((self.sigma_val[2] + self.TINY) * 2 ** 0.5)))

            weighted_likelihoods = likelihoods_0*self.weight[0] + likelihoods_1*self.weight[1] + likelihoods_2*self.weight[2]
            freq = int(math.floor((weighted_likelihoods) * self.mul_factor) + 1)
    ...
    def get_low(self, symbol):
        ...
        c = (0.5 * (1 + scipy.special.erf(((symbol-1) + 0.5 - self.mu_val[0]) / ((self.sigma_val[0] + self.TINY) * 2 ** 0.5))))*self.weight[0] + \
            (0.5 * (1 + scipy.special.erf(((symbol-1) + 0.5 - self.mu_val[1]) / ((self.sigma_val[1] + self.TINY) * 2 ** 0.5))))*self.weight[1] + \
            (0.5 * (1 + scipy.special.erf(((symbol-1) + 0.5 - self.mu_val[2]) / ((self.sigma_val[2] + self.TINY) * 2 ** 0.5))))*self.weight[2]
        c = int(math.floor(c * self.mul_factor) + symbol)
        return c
    ...
    def get_high(self, symbol):
        ...
        c = (0.5 * (1 + scipy.special.erf(((symbol) + 0.5 - self.mu_val[0]) / ((self.sigma_val[0] + self.TINY) * 2 ** 0.5))))*self.weight[0] + \
            (0.5 * (1 + scipy.special.erf(((symbol) + 0.5 - self.mu_val[1]) / ((self.sigma_val[1] + self.TINY) * 2 ** 0.5))))*self.weight[1] + \
            (0.5 * (1 + scipy.special.erf(((symbol) + 0.5 - self.mu_val[2]) / ((self.sigma_val[2] + self.TINY) * 2 ** 0.5))))*self.weight[2]
        c = int(math.floor(c * self.mul_factor) + symbol+1)
...
class BitOutputStream(object):
    ...
    # padding is set to True by default
    def close(self):
